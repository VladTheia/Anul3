	...
	public void run() {
		...
		while (true) {
			try {
				...
				if (headQuarterMessage.getData().equals(HeadQuarter.END)) {
					lockSpaceExplorer.release();
					continue;
				}
				...
				// it the odd message is't END, than it's a parent, which means the even message is its child
				Message childMessage = channel.getMessageHeadQuarterChannel();
				lockSpaceExplorer.release();
				// as it's a shared set, we need to avoid concurrency
				synchronized (discovered) {
					if (!discovered.contains(childMessage.getCurrentSolarSystem())) {
						discovered.add(childMessage.getCurrentSolarSystem());
						// decrypt the data and sent it to the HQs
